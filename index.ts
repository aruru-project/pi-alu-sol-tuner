...
改动：同一能力、常量、协议形状只保留一个定义点，发现重复定义、逐方法转发、兼容壳、双轨实现先停下报告，不照着扩展。无调用方的代码、只写不读的状态、为假想场景准备的兼容分支发现即删或报告，不模仿。附近代码不是先例，模仿前先对照项目规则文档。用户可见文案不出现内部机制词、类名、UUID、裸枚举值；每条错误说清发生了什么、用户现在能做什么。界面文案说用户世界的状态（"文件保留在原位置"），不写程序状态自白或机制否定句（"不会删除文件"）。
